...
for (let i = 1; i <= total; i++) {
  ...
  const labelData = {
    ...
    expiryDate: isBadA ? '30/09/26' : '30/09/2026', // V006
    netWeightG: 100,
    ingredientList: isBadB ? ['Apple', 'Gelatine'] : ['Apple'], // V005 missing allergen bold, V007 halal needed for gelatine
    allergenDeclaration: [],
    mayContainAdvisory: [],
    fssaiLicenseNumber: undefined,
  ...
  packets.push({
    ...
    formulation: {
        ...
        nutritionPanel: {
            ...
            declaredTotalG: isBadB ? 102.3 : 100 // V002 arithmetic error
        },
        coloringAgents: [],
        artificialFlavors: [],
        productionLineId: "LINE-SHARED-PEANUT",
        sharedLineProducts: ["Peanut Butter Mash"] // V004 no may-contain
    },
    regulatory: {
        ...
        halalCertRequired: isBadB, // depends on gelatine presence
        halalCertPresent: false, // V007 missing cert
  ...
  if (isBadA) {
    violations.push(
      ...
      { violationCode: 'VC-LABEL-004', packetId: \`PKT-MED-\${i}\`, lotNumber: lot, severity: 'minor', correctActions: ['REPRINT_EXPIRY_DATES', 'APPLY_CORRECTED_EXPIRY_STICKER', 'RECALL_AND_RELABEL_EXPIRY'], affectsEntireBatch: false, isArithmeticViolation: false, cascadeTriggers: [] } // Fake expiry code for V006
    )
  }
  if (isBadB) {
     violations.push(
