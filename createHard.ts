...
// LOT-X: 80 packets
for (let i = 1; i <= 80; i++) {
  const isBad = i <= 2; // only put violations on first 2 to make it discoverable safely
  if (isBad) {
      violations.push(
  ...
  packets.push({
    ...
    labelData: {
      ...
      fssaiLicenseNumber: undefined, fdaRegistrationNumber: isBad ? 'INVALID-123' : 'FDA-VALID-999',
...
for (let i = 1; i <= 60; i++) {
  ...
  packets.push({
    ...
    labelData: {
      ...
      allergenDeclaration: [], mayContainAdvisory: [],
      fssaiLicenseNumber: undefined, fdaRegistrationNumber: 'FDA-VALID-999',
      hasNutritionLabel: true, languageCompliant: !isBad, hasQRCode: true, // languageCompliant false = V008 Spanish missing
    },
    formulation: {
        ...
        coloringAgents: [], artificialFlavors: isBad ? ['Cyclamate (E952)'] : [], productionLineId: "LINE-SHARED-PEANUT", sharedLineProducts: [] // hidden in flavor
    },
    regulatory: {
        destinationMarket: 'US',
...
// LOT-Z: 60 packets
for (let i = 1; i <= 60; i++) {
  const isBad = i <= 3; // 3 packets missing expiry
  if (isBad) {
      violations.push(
